(function ($, port) {
    ...
    function Controller(id){
        ...
        this.unready = function(){
            if(typeof this.readyModal != "undefined")
                this.readyModal.hide();
            if(typeof this.pendingModal != "undefined")
    ...
    function Room(id){
         ...
         */
        this.render = function(){
            console.error("Room:render not implemented");
        };
    }
    ...
    Room.prototype.render = function(){
        if($("#room_" + this.id).length == 0){
            $("#rooms").append(
                "<div id='room_" + this.id + "' class='room col-xl-6 col-lg-6 col-md-6 col-sm-6 col-12'>"+
                "</div>"
            );
        }
        ...
        $("#room_" + this.id).html(
            "<div class='room-header'>Mesa "+ this.id + "</div>"+
            "<div class='room-content'>"+
                "<button class='join_room' data-room='" + this.id + "'>Entrar</button>"+
                players_html +
            "</div>"
        );
    }
    ...
    socket.on("successAuth",   function(data){
        ...
        $.each(data.players, function(){
            controller.add_player(controller.createPlayer(this));
        });

        $.each(data.rooms, function(room){
            controller.add_room(controller.createRoom(this));
        });
        ...
        socket.on("play", function(data){
            ...
            $.each(data.players, function(i, player){
                var color = new Color(
                    player.color.id, player.color.name,
                    player.color.initial, player.color.breaker,
                    player.color.postbreak, player.color.finish,
                ...
                $.each(player.chips, function(j, chip){
                    var chip = new Chip(
                        chip.id, chip.position, color, game,
            ...
            socket.on("dices",        game.request_dices);
            socket.on("info_dices",   game.info_dices);
            socket.on("move",         game.request_move);
            socket.on("skip_move",    game.skip_move);
            socket.on("skip_double",  game.skip_double);
            socket.on("info_move",    game.info_move);
            socket.on("info_die",     game.info_die);

            $(".dices").on("click", game.throw_dices);
        });
